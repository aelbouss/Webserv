                      ...
                      const std::vector<char>& requestBody)
{
    ...
    short errorCode = 0;
    cgi.initEnvFromResponse(method, queryString, requestBody);
    ...
    if (method == "POST" && !requestBody.empty())
        wroteOk = writeAll(cgi.pipe_in[1], &requestBody[0], requestBody.size());
    close(cgi.pipe_in[1]);

    std::string output = readAll(cgi.pipe_out[0]);
    close(cgi.pipe_out[0]);

    int status = 0;
    if (waitpid(cgi.getCgiPid(), &status, 0) < 0)
    {
        buildErrorPage(500);
        return;
    }

    if (!wroteOk || output.empty() || !WIFEXITED(status))
    {
        buildErrorPage(500);
        return;
    }
